...
%\begin{equation}
  %\label{eq:ddistr}
 % d(m_{i,j}) \sim N\left[1+\bm{g}(m_{i,j})'\bm{\beta};\bm{g}(m_{i,j})'\left(\sigma^2(\bm{X' X})^{-1} \right)\bm{g}(m_{i,j}) \right],
%\end{equation} 


 The $1- \alpha$ confidence interval for $\mu_{\delta}$ of the discount function can now be constructed the usual way
