...
We substitute \eqref{dfmatrixspline} in \eqref{fairpricespline} and get an expression which is linear in the paramter vector $\bm{\beta}=(\beta^1,\dots,\beta^k)$.
...
\bm{\hat \beta}_{\left[k \times 1\right]}= \left( \bm{X}^t   \bm{X}\right )^{-1}\bm{X}^t \bm{y}
\end{equation}
