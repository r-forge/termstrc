...
$+\bm{\epsilon}$ in discount function?
...
Under the assumption of a normal distributed disturbence term $\bm{\epsilon}$ the conditional 
distribution of $\bm{b}$ given $\bm{X}$ can be expressed as
...
\end{equation}
If the elements of $\bm{\epsilon}$ are independently identically distributed with mean zero and finite
variance $\sigma^2$ and $x_{iK}$ is such that the Grenander conditions are met, 
then the asymptotic distribution fo $\bm{b}$ is
...
Economically $d(m_{i,j})>0 \qquad  \forall m_{i,j}> 0$.
Therefore a we suggest a truncated normal distribution for $d(m_{i,j})$

 
The denisty of the truncated normal distribution can be written as
...
Var[d(m_{i,j})|d(m_{i,j})>0] = E\left[d(m_{i,j})^2|d(m_{i,j})\right] = M''(t)|_{t=0} = \sigma^2\left[1+ \frac{\frac{-\mu}{\sigma}\phi(\frac{-\mu}{\sigma})}{1-\Phi(\frac{-\mu}{\sigma})} -\left(\frac{\phi(\frac{-\mu}{\sigma})}{1-\Phi(\frac{-\mu}{\sigma})^2} \right)\right]
