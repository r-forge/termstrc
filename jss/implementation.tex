...
For that purpose, the package \pkg{termstrc} provides two core functions and several helper functions. \code{nelson\_estim()} performs the estimation according to the parametric \cite{Nelson1987} method with the \cite{Svensson1994} extension described in Section~\ref{sec:nels-svenss-meth}.  \code{splines\_estim()} allows the user to estimate the term structure according to the \cite{McCulloch1975} cubic splines approach mentioned in Section~\ref{sec:cubic-splines}. Both of the core functions require a minimum amount of input. Table~\ref{tab:corefct} gives an overview about the similar types of arguments.
...
The parameters of the cubic splines approach are estimated with OLS by using the \code{lm()} function. Consequently, \code{summary.cubicsplines()} also provides summary statistics for the OLS estimation by applying \code{summary.lm()} to the list elements of \code{<object>\$regout}.
