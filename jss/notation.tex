...
The simplest smoothing technique is linear interpolation. In many cases it is not appropriate for zero-coupon yield curve estimation, because it can lead to spikes in the forward rate curve, which is a problem from an economic point of view.

The next two sections present two popular ways to specify a form for the spot rate or the discount function and solve the optimization problem in \eqref{eq:optimalparam}.
